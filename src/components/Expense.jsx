...
const Expense = ({ expenseDatas, setExpenseDatas, onEdit, onDelete }) => {
    ...
    const sortLowToHigh = () => {
      const sorted = [...expenseDatas].sort((a, b) => a.amount - b.amount);
      setExpenseDatas(sorted);
    };

    // Function to sort high to low
    const sortHighToLow = () => {
      const sorted = [...expenseDatas].sort((a, b) => b.amount - a.amount);
      setExpenseDatas(sorted);
    };
    ...
    const handleExpenseSort = () => {
      setShowExpenseSort((prevState) => !prevState);
    };

    // Toggle filter menu
    const handleExpenseFilter = () => {
      setShowExpenseFilter((prevState) => !prevState);
    };
    return (
      <div>
          ...
          <div className="flex items-center justify-between gap-2 bg-[#F9FAFB] py-4 px-4 rounded-md">
            ...
            <div className="flex gap-2">
              ...
              <div className="relative inline-block">
                ...
                {showExpenseFilter && (
                  <div className="absolute z-10 mt-2 w-56 bg-white shadow-lg rounded-md">
                    {["Education", "Food", "Health", "Bill","Insurance","Tax","Transport","Telephone"].map(
                      (category, index) => (
                        <label
                          key={index}
            ...
            {filteredData.map((expenseData) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-gray-600">{expenseData.date}</p>
                </div>
                <div className="flex items-center gap-2">
                  <p className="text-base font-semibold text-gray-600 transition-all group-hover:-translate-x-14">
                    BDT {expenseData.amount}
                  </p>
